...
int  main()
{
	...
	uc = 'B'; // 
	uc = -1; // /xff (255 неявное преобразование)
	stop;
	...
	l = FLT_MAX_EXP;	// макс степень числа float
	...
	c = s = i = f = d = 100 / 3; // 33 (как значение) присвается к char а потом ->
	c = s = i = f = d = 10 / 3; // аналогично только с 3
	c = s = i = f = d = 1 / 3.; // 1 / 3 получается double присваиваем 0.33 а затем <-

	// Пример "небрежного" использования неявного приведения типов.	 Объясните результаты.
	i = 256;
	c = i; // 0 т.к переполнение char 8 бит
